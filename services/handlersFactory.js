...
exports.deleteOne = (Model) =>
  asyncHandler(async (req, res, next) => {
    ...
    if (!document) {
      return next(new ApiError(`No ${Model} For this ${id}`, 404));
    }
    // Trigger remove Event when update document
    // remove event (which in reviewModel.js)
...
exports.updateOne = (Model) =>
  asyncHandler(async (req, res, next) => {
    ...
    // save event (which in reviewModel.js)
    document.save();
...
exports.getAll = (Model, modelNameForSearch = "") =>
  asyncHandler(async (req, res) => {
    let filter = {};
    if (req.filterObj) {
